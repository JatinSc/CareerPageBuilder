...
export default function Preview() {
  ...
  useEffect(() => {
    const fetchPreview = async () => {
      try {
        const res = await api.get("/api/company/preview");
        setData(res.data);
      } catch (err) {
  ...
  const primaryBg = branding.primaryBackground || "#ffffff";
  const secondaryBg = branding.secondaryBackground || "#f3f4f6";
  const primaryText = branding.primaryText || "#000000";
  const secondaryText = branding.secondaryText || "#4b5563";
  ...
  return (
    ...
    >
      <div className="fixed top-6 right-6 z-50 animate-in fade-in slide-in-from-top-4 duration-500 flex items-center gap-3">
        <div className={`flex items-center gap-2 px-4 py-2 rounded-full font-bold text-sm shadow-lg border backdrop-blur-md ${company.published ? "bg-green-100/90 text-green-700 border-green-200" : "bg-amber-100/90 text-amber-700 border-amber-200"}`}>
          {company.published ? <CheckCircle size={16} /> : <FileEdit size={16} />}
          <span>{company.published ? "Published" : "Draft Mode"}</span>
        </div>
        <button
          onClick={async () => {
            try {
              const shareUrl = `${window.location.origin}/${company.slug}/careers`;
          ...
          className="flex items-center gap-2 px-3 py-2 rounded-full bg-white text-gray-700 border border-gray-200 shadow-sm hover:bg-gray-50 transition-colors"
        >
          <Share2 size={16} />
      ...
      <div className="max-w-6xl mx-auto px-6 mt-10 relative z-20 pb-12">
        {/* Sections */}
              ...
              return (
                <div key={s._id} className="relative rounded-3xl overflow-hidden min-h-[500px] flex items-center justify-center text-center text-white shadow-2xl">
                  {embed.type === 'direct' ? (
                    ...
                    <iframe
                      src={embed.url}
                      className="absolute inset-0 w-full h-full object-cover pointer-events-none scale-[1.5]"
                      frameBorder="0"
                      allow="autoplay"
                    ></iframe>
                  ) : (
              ...
              return (
                <div key={s._id} className={`flex flex-col md:flex-row items-center gap-12 ${isVideoRight ? 'md:flex-row-reverse' : ''}`}>
                  <div className="w-full md:w-1/2">
                    <div className="relative rounded-2xl overflow-hidden shadow-xl transform transition-transform hover:scale-[1.02] duration-300 aspect-video bg-black">
                      {embed.type === 'direct' ? (
                        ...
                        <iframe
                          src={(() => {
                            if (embed.type === 'google_drive') return embed.url;
                            if (embed.type === 'youtube') return `${embed.url}?autoplay=1&mute=1&controls=0&loop=1&playlist=${embed.id}&background=1`;
                            if (embed.type === 'vimeo') return `${embed.url}?background=1&autoplay=1&muted=1&loop=1`;
                            return embed.url;
                          })()}
                  ...
                  <div className="w-full md:w-1/2 text-center md:text-left">
                    ...
                    <div className="text-xs lg:text-[1rem] prose prose-lg text-gray-600 leading-relaxed whitespace-pre-wrap max-sm:prose-sm">
                      {s.content}
                    </div>
                  </div>
                </div>
